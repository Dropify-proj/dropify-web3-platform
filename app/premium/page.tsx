...
import { useTelegram, TelegramHeader, TelegramButton } from '../components/TelegramMiniApp';
...
export default function PremiumDashboard() {
  ...
  const { premiumStatus, premiumPricing } = useTelegramStarsPremium(adminAddress, userAddress);
  const isUserPremium = PremiumUtils.isUserPremium(premiumStatus);

  useEffect(() => {
    if (isReady && isTelegramWebApp && webApp) {
      webApp.MainButton.setText('🚀 Start Earning Premium Rewards');
  ...
  }, [isReady, isTelegramWebApp, webApp, navigateWithHaptic]);
